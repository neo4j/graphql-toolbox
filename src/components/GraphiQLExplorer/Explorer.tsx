...
type MakeDefaultArg = (parentField: Field, arg: GraphQLArgument | GraphQLInputField) => boolean;
...
type Props = {
    ...
    getDefaultFieldNames?: ((type: GraphQLObjectType) => Array<string>) | null | undefined;
        ...
        try {
            switch (argType.name) {
                case "String":
                    return {
                        kind: Kind.STRING,
                        value: String(argType.parseValue(stringValue)),
                    };
                case "Float":
                    return {
                        kind: Kind.FLOAT,
                        value: String(argType.parseValue(parseFloat(stringValue))),
                    };
                case "Int":
                    return {
                        kind: Kind.INT,
                        value: String(argType.parseValue(parseInt(stringValue, 10))),
                    };
                case "Boolean":
                    try {
            ...
            return { kind: Kind.STRING, value: stringValue };
        }
    } else {
        try {
            const parsedValue = argType.parseValue(stringValue);
            if (parsedValue) {
...
type InputArgViewProps = {
    ...
    parentField: Field;
    modifyFields: (fields: readonly ObjectFieldNode[], commit: boolean) => DocumentNode | null;
...
class InputArgView extends React.PureComponent<InputArgViewProps> {
    ...
    _addArg = () => {
        const { selection, arg, getDefaultScalarArgValue, parentField, makeDefaultArg } = this.props;
        ...
        if (!argSelection) {
            ...
            return this.props.modifyFields([...(selection.fields || []), argSelection], true);
        }
    };

    _setArgValue = (event: unknown, options: { commit: boolean } | null | undefined) => {
        ...
        try {
            ...
            // Do nothing, just want to catch cases where event doesn't have a kind or target
        }
        ...
        const handleable = isLeafType(argType) || settingToVariable || settingToNull || settingToLiteralValue;
        ...
        let value: ValueNode | null | undefined;
...
type ArgViewProps = {
    ...
    arg: GraphQLArgument;
    selection: FieldNode;
    modifyArguments: (argumentNodes: readonly ArgumentNode[], commit: boolean) => DocumentNode | null;
    getDefaultScalarArgValue: GetDefaultScalarArgValue;
    makeDefaultArg: MakeDefaultArg | null | undefined;
    onRunOperation: () => void;
    styleConfig: StyleConfig;
...
class ArgView extends React.PureComponent<ArgViewProps, ArgViewState> {
    ...
    _addArg = (commit: boolean): DocumentNode | null => {
        ...
        if (!argSelection) {
            ...
            return this.props.modifyArguments([...(selection.arguments || []), argSelection], commit);
        }
    };
    _setArgValue = (
        event: React.ChangeEvent | VariableDefinitionNode,
        options: { commit: boolean } | null | undefined
    ) => {
        let settingToNull = false;
        let settingToVariable = false;
        let settingToLiteralValue = false;
...
function canRunOperation(operationName: string) {
...
type ScalarInputProps = {
    arg: GraphQLArgument;
    argValue: ValueNode;
    setArgValue: (event: React.ChangeEvent | VariableDefinitionNode, commit: boolean) => DocumentNode | null;
    onRunOperation: () => void;
    styleConfig: StyleConfig;
};

class ScalarInput extends React.PureComponent<ScalarInputProps> {
    _ref: unknown | null | undefined;
    ...
    render() {
        ...
        const color = this.props.argValue.kind === Kind.STRING ? styleConfig.colors.string : styleConfig.colors.number;
        return (
            <span style={{ color }}>
                {argType.name === "String" ? '"' : ""}
                <input
...
type AbstractArgViewProps = {
    ...
    parentField: Field;
    setArgValue: (event: React.ChangeEvent | VariableDefinitionNode, commit: boolean) => DocumentNode | null;
    setArgFields: (fields: readonly ObjectFieldNode[], commit: boolean) => DocumentNode | null;
...
class AbstractArgView extends React.PureComponent<AbstractArgViewProps, { displayArgActions: boolean }> {
    ...
    render() {
                ...
                if (argType.name === "Boolean") {
                    input = (
                        <select
                            ...
                            value={argValue.kind === Kind.BOOLEAN ? argValue.value : undefined}
                        >
      ...
      */
            ...
            const conflictingNameCount = (this.props.definition.variableDefinitions || []).filter((varDef) =>
            ...
            const variableDefinitionByName = (name) =>
                (this.props.definition.variableDefinitions || []).find((varDef) => varDef.variable.name.value === name);

            let variable: VariableDefinitionNode | null | undefined;
            ...
            if (variable) {
                const newDoc: DocumentNode | null | undefined = this.props.setArgValue(variable, false);
                ...
                if (newDoc) {
                    ...
                    const newVariableDefinitions: Array<VariableDefinitionNode> = [
                        ...
                        variable,
                    ].filter((varDef) => newlyUnusedVariables.indexOf(varDef.variable.name.value) === -1);
        ...
        const devariablize = () => {
            ...
            const variableDefinition = (this.props.definition.variableDefinitions || []).find(
            ...
            if (newDoc) {
                const targetOperation: OperationDefinitionNode | null | undefined = newDoc.definitions.find(
                    (definition) => definition.name.value === this.props.definition.name.value
                );
            ...
            >
                ...
                >
                    {isInputObjectType(argType) ? (
                        <span>{argValue ? this.props.styleConfig.arrowOpen : this.props.styleConfig.arrowClosed}</span>
                    ) : (
...
type AbstractViewProps = {
    ...
    modifySelections: (selections: Selections, options?: { commit: boolean }) => DocumentNode | null;
...
class AbstractView extends React.PureComponent<AbstractViewProps> {
        ...
        this.props.modifySelections([
            ...
            this._previousSelection || {
                ...
                selectionSet: {
                    ...
                    selections: this.props.getDefaultFieldNames(this.props.implementingType).map((fieldName) => ({
                        kind: Kind.FIELD,
    ...
    render() {
        const { implementingType, schema, getDefaultFieldNames, styleConfig } = this.props;
        const selection = this._getSelection();
        const fields = implementingType.getFields();
        const childSelections = selection ? (selection.selectionSet ? selection.selectionSet.selections : []) : [];

        return (
            <div className={`graphiql-explorer-${implementingType.name}`}>
                <span style={{ cursor: "pointer" }} onClick={selection ? this._removeFragment : this._addFragment}>
        ...
        this.props.modifySelections(
            this.props.selections.filter((s) => {
                const isTargetSelection =
                    s.kind === "FragmentSpread" && s.name.value === this.props.fragment.name.value;

                return !isTargetSelection;
            })
        );
    };
    _getSelection = (): FragmentSpreadNode | null | undefined => {
        const selection = this.props.selections.find((selection) => {
            return selection.kind === "FragmentSpread" && selection.name.value === this.props.fragment.name.value;
        });
...
class FieldView extends React.PureComponent<FieldViewProps, { displayFieldActions: boolean }> {
    ...
    _removeFieldFromSelections = () => {
        ...
        this.props.modifySelections(this.props.selections.filter((selection) => selection !== previousSelection));
    };
    _getSelection = (): FieldNode | null | undefined => {
        const selection = this.props.selections.find(
            (selection) => selection.kind === "Field" && this.props.field.name === selection.name.value
        );
        if (!selection) {
            return null;
        }
    ...
    _setArguments = (argumentNodes: readonly ArgumentNode[], options?: { commit: boolean }): DocumentNode | null => {
        const selection = this._getSelection();
        if (!selection) {
    ...
    _modifyChildSelections = (selections: Selections, options?: { commit: boolean }): DocumentNode | null => {
        return this.props.modifySelections(
            this.props.selections.map((selection) => {
    ...
    render() {
        ...
        const node = (
            <div className={className}>
                <span
                    ...
                    onMouseEnter={() => {
                        const containsMeaningfulSubselection =
                            ...
                            selection.selectionSet.selections.filter((selection) => selection.kind !== "FragmentSpread")
                                .length > 0;
                ...
                >
                    {isObjectType(type) ? (
                        <span>{selection ? this.props.styleConfig.arrowOpen : this.props.styleConfig.arrowClosed}</span>
                    ) : null}
                    {isObjectType(type) ? null : (
                        <Checkbox checked={!!selection} styleConfig={this.props.styleConfig} />
                    )}
                    <span style={{ color: styleConfig.colors.property }} className="graphiql-explorer-field-view">
                        {field.name}
                    </span>
                    {!this.state.displayFieldActions ? null : (
                        <button
                            ...
                            onClick={(event) => {
                                ...
                                const newDoc = this._modifyChildSelections(nextSelections, false);
                ...
                {selection && args.length ? (
                    <div style={{ marginLeft: 16 }} className="graphiql-explorer-graphql-arguments">
                        {args.map((arg) => (
                            <ArgView
                                key={arg.name}
                                parentField={field}
...
function parseQuery(text: string): DocumentNode | null | Error {
    try {
        if (!text.trim()) {
            return null;
        }
...
type RootViewProps = {
    schema: GraphQLSchema;
    isLast: boolean;
    fields: GraphQLFieldMap<unknown, unknown> | null | undefined;
    operationType: OperationType;
    name: string | null | undefined;
    onTypeName: string | null | undefined;
    definition: FragmentDefinitionNode | OperationDefinitionNode;
    onEdit: (
        operationDef: OperationDefinitionNode | FragmentDefinitionNode | null | undefined,
        commit: boolean
    ) => DocumentNode;
...
> {
    state = { newOperationType: "query" as NewOperationType, displayTitleActions: false };
    _previousOperationDef: OperationDefinitionNode | FragmentDefinitionNode | null | undefined;

    _modifySelections = (selections: Selections, options?: { commit: boolean }): DocumentNode => {
        ...
        let newOperationDef: OperationDefinitionNode | FragmentDefinitionNode | null | undefined;
    ...
    render() {
        ...
        const operationDisplayName = this.props.name || `${capitalize(operationType)} Name`;
...
function Attribution() {
    return (
        ...
        >
            ...
            <div>
                Contribute on <a href="https://github.com/OneGraph/graphiql-explorer">GitHub</a>
            </div>
        </div>
    );
}
...
class Explorer extends React.PureComponent<Props, State> {
    ...
    render() {
        ...
        const getDefaultScalarArgValue = this.props.getDefaultScalarArgValue || defaultGetDefaultScalarArgValue;
        ...
        const renameOperation = (
            ...
            const newName = name == null || name === "" ? null : { kind: Kind.NAME, value: name, loc: undefined };
        ...
        const cloneOperation = (targetOperation: OperationDefinitionNode | FragmentDefinitionNode) => {
            let kind;
        ...
        const destroyOperation = (targetOperation: OperationDefinitionNode | FragmentDefinitionNode) => {
            const existingDefs = parsedQuery.definitions;
        ...
        const addOperation = (kind: NewOperationType) => {
            ...
            const viewingDefaultOperation =
                parsedQuery.definitions.length === 1 && parsedQuery.definitions[0] === DEFAULT_DOCUMENT.definitions[0];
            ...
            actionsOptions.length === 0 || this.props.hideActions ? null : (
                ...
                >
                    ...
                    >
                        ...
                        <select
                            onChange={(event) => this._setAddOperationType(event.target.value as NewOperationType)}
                        ...
                        <button
                            ...
                            className="toolbar-button"
                            onClick={() =>
                                this.state.newOperationType ? addOperation(this.state.newOperationType) : null
                            }
                            style={{
        ...
        const externalFragments =
            ...
            this.props.externalFragments.reduce((acc, fragment) => {
                if (fragment.kind === "FragmentDefinition") {
                    ...
                    const newFragmentsForType = [...existingFragmentsForType, fragment].sort((a, b) =>
        ...
        const documentFragments: AvailableFragments = relevantOperations.reduce((acc, operation) => {
            if (operation.kind === "FragmentDefinition") {
                const fragmentTypeName = operation.typeCondition.name.value;
                const existingFragmentsForType = acc[fragmentTypeName] || [];
                const newFragmentsForType = [...existingFragmentsForType, operation].sort((a, b) =>
                    a.name.value.localeCompare(b.name.value)
                );
                return {
                    ...acc,
        ...
        return (
            ...
            >
                ...
                >
                    {relevantOperations.map((operation: OperationDefinitionNode | FragmentDefinitionNode, index) => {
                        ...
                        const fragmentFields =
                            fragmentType instanceof GraphQLObjectType ? fragmentType.getFields() : null;
                        ...
                        const fragmentTypeName =
                            operation.kind === "FragmentDefinition" ? operation.typeCondition.name.value : null;
                        ...
                        return (
                            <RootView
                                ...
                                onCommit={onCommit}
                                onEdit={(
                                    newDefinition: DefinitionNode | null | undefined,
